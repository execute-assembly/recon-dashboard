...
bash recon-files/port_scan.sh                  || die "Stage 3 (port scan) failed"
[[ -s probe/port-scan/domain_ips.json ]]       || die "Stage 3 produced no IP mappings — aborting"

bash recon-files/alive_httpx_probe.sh          || die "Stage 4 (HTTP probe) failed"
